...
def ECCV(image, is_training=False, val = False, lr = None, prediction_fn=slim.softmax,scope='ECCV'):
    with tf.variable_scope(scope, 'ECCV', [image]):
        image = slim.batch_norm(image, scale=False,activation_fn=None,scope='batch0', is_training=is_training, reuse = val)
        image = slim.conv2d(image, 64, [3, 3], 2, padding = 'SAME', activation_fn=None,
                               scope='conv0', trainable=is_training, reuse=val)
        image = slim.batch_norm(image, scale=False,activation_fn=tf.nn.relu,scope='batch1', is_training=is_training, reuse = val)
        ...
        with tf.variable_scope('block0'):
            ...
            x = resi_block(x,128,is_training=is_training, val=val, name = 'conv2')
            x = resi_block(x,128,is_training=is_training, val=val, name = 'conv3')
            x = resi_block(x,128,is_training=is_training, val=val, name = 'conv4')
        ...
        with tf.variable_scope('block1'):
            ...
            x = resi_block(x,256,is_training=is_training, val=val, name = 'conv2')
            x = resi_block(x,256,is_training=is_training, val=val, name = 'conv3')
            x = resi_block(x,256,is_training=is_training, val=val, name = 'conv4')
        ...
        with tf.variable_scope('block2'):
            ...
            x = resi_block(x,512,is_training=is_training, val=val, name = 'conv2')
            x = resi_block(x,512,is_training=is_training, val=val, name = 'conv3')
            x = resi_block(x,512,is_training=is_training, val=val, name = 'conv4')
