...
def _get_output_filename(dataset_dir, split_name):
    return '%s/imgnet64%s.tfrecord' % (dataset_dir, split_name)
        ...
        with tf.python_io.TFRecordWriter(training_filename) as tfrecord_writer:
            ...
            with tf.Session('',config=tf.ConfigProto(log_device_placement=True)) as sess:
                ...
                for batch in range(1,11):
                    with open('/home/dmsl/Documents/data/imgnet64/train_data_batch_%d'%batch, 'rb') as fo:
                        img_queue = pickle.load(fo, encoding='bytes')
                    
                    for n in xrange(img_queue['data'].shape[0]):
                        image = img_queue['data'][n]
                        image = np.transpose(image.reshape(3,64,64),(1,2,0))
                        label = img_queue['labels'][n]
                        if label < 100:
                            ...
                            sys.stdout.write('\r>> Reading dataset images %d/%d' 
                                             % (n+dataset_len , dataset_len+img_queue['data'].shape[0]))
                    dataset_len += img_queue['data'].shape[0]
