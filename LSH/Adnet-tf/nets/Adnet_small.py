...
def Adnet_small_arg_scope(weight_decay=0.0005):
...
def Adnet_small(image, is_training=False, val = False, vd = None, prediction_fn=slim.softmax,scope='Adnet_small'):
    with tf.variable_scope(scope, 'Adnet_small', [image]):
        ...
        conv = slim.conv2d(conv, 256, [3, 3], 2, padding = 'VALID', activation_fn=tf.nn.relu,
...
#        conv = gradient_scale(conv, 0.1)
        ...
        fc1 = slim.fully_connected(conv, 512, activation_fn=tf.nn.relu, trainable=is_training, scope = 'full', reuse = val)
        fc1 = slim.dropout(fc1,0.5,is_training=is_training)
        
        act = fully_connected_layers(fc1, 11, video_num, is_training = is_training, scope = 'full_act', reuse = val)
        conf = fully_connected_layers(fc1, 2, video_num, is_training = is_training, scope = 'full_conf', reuse = val)
    ...
    return end_points
Adnet_small.default_image_size = 32
